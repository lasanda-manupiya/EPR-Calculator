  ...
  return <div className="space-y-4"><h2 className="text-2xl font-semibold">Packaging Library</h2>
    <div className="bg-white rounded-xl p-4 shadow space-y-3"><h3 className="font-semibold">Add or Edit reference</h3>
      ...
      <div className="grid md:grid-cols-4 gap-2">
        ...
        <select disabled={!canEditLibrary} className="border rounded px-3 py-2 disabled:bg-slate-100" value={form.materialType} onChange={e => setForm({ ...form, materialType: e.target.value as MaterialType })}>{['Cardboard','Plastic','Paper','Glass','Aluminium','Steel','Wood','Other'].map(m => <option key={m}>{m}</option>)}</select>
        <select disabled={!canEditLibrary} className="border rounded px-3 py-2 disabled:bg-slate-100" value={form.packagingType} onChange={e => setForm({ ...form, packagingType: e.target.value as PackagingType })}><option value="primary">Primary</option><option value="secondary">Secondary</option><option value="tertiary">Tertiary</option></select>
        <input disabled={!canEditLibrary} className="border rounded px-3 py-2 disabled:bg-slate-100" type="number" placeholder="Avg weight (g)" value={form.averageWeight} onChange={e => setForm({ ...form, averageWeight: Number(e.target.value) || 0 })} />
        {(['length','width','height'] as const).map(k => <input disabled={!canEditLibrary} key={k} className="border rounded px-3 py-2 disabled:bg-slate-100" type="number" placeholder={k} value={form[k]} onChange={e => setForm({ ...form, [k]: Number(e.target.value) || 0 })} />)}
        <input disabled={!canEditLibrary} className="border rounded px-3 py-2 md:col-span-2 disabled:bg-slate-100" placeholder="Notes" value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} />
      </div>
      <div className="flex gap-2"><button disabled={!canEditLibrary} className="px-3 py-2 bg-eco text-white rounded disabled:opacity-50" onClick={submit}>{editingId ? 'Update reference' : 'Add reference'}</button>{editingId && <button className="px-3 py-2 border rounded" onClick={() => { setEditingId(null); setForm(emptyForm); }}>Cancel edit</button>}</div>
    </div>
